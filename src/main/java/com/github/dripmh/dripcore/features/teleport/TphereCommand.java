...
public class TphereCommand extends BaseCommand {
    ...
    public TphereCommand() {
        super( "tphere", YMLUtils.getConfig().getStr( "teleport.tphere-perm" ), "&8/&7tphere &b(player)" );
        register( true );
    }
        ...
        if ( demandPlayer( sender ) == false ) { return true; }
            ...
            if ( CoreUtils.isValidUsername( args[0] ) ) {
                ...
                if ( target != null ) {
                    ...
                    staff.sendMessage( CoreUtils.getColored( "&7You have teleported &b" + target.getName() + "&7 to yourself" ) );

                    for ( Player p : Bukkit.getOnlinePlayers() ) {
                        if ( VaultUtils.hasPerms( p, YMLUtils.getConfig().getStr( "teleport.tpnotifs-perm" ) ) && p.getName().equals( staff.getName() ) == false ) {
                            p.sendMessage( CoreUtils.getColored( String.format( "&7[%s teleported %s to themselves]", staff.getName(), target.getName() ) ) );
                        }
                    }
